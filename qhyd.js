...
[MITM]
hostname = www.qianhe5.com,www.qliang2.com
...
key = key.urlKey
let url = {
        url : `https://www.qliang2.com/read/v1/get_time_list_task`,
        headers : JSON.parse(qhydhd),
...
let url = {
        url : 'https://www.qliang2.com/read/v1/get_click_task',
        headers : JSON.parse(qhydhd),
        body : `{"urlKey":"${key}","task_id":${id},"type":1,"str_id":"${uid}"}`,
}
      $.post(url, async (err, resp, data) => {
        try {
    const result = JSON.parse(data)
...
let url = {
        url : `https://www.qliang2.com/read/v1/get_click_task`,
        headers : JSON.parse(qhydhd),
        body : `{"urlKey":"${key}","task_id":${id},"time":${times},"type":2,"str_id":"${strid}"}`,
}
      $.post(url, async (err, resp, data) => {
        try {
    const result = JSON.parse(data)
